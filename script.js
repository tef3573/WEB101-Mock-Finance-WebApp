...
document.addEventListener("DOMContentLoaded", () => {
  ...
  themeToggle.addEventListener("click", () => {
    ...
    // Update button label so it’s clear what clicking will do next
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function isValidEmail(value) {
    // simple email regex – good enough for this project
  ...
  form.addEventListener("submit", (e) => {
    ...
    // Reset errors
    [nameInput, emailInput, roleInput, notesInput].forEach(clearError);
    ...
    if (!roleVal) {
      setError(roleInput, "Please tell us your affiliation or role.");
      valid = false;
    }

    // If anything is invalid, do not add RSVP
    if (!valid) {
      return;
    }

    // Valid: add to list
    if (rsvpList.querySelector(".rsvp-empty")) {
      rsvpList.removeChild(rsvpList.querySelector(".rsvp-empty"));
    }

    const li = document.createElement("li");
    li.innerHTML = `
      <strong>${nameVal}</strong>
      <span>${roleVal} • ${emailVal}</span>
      ${notesVal ? `<span>Note: ${notesVal}</span>` : ""}
    `;
